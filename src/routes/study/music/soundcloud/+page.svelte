...
<section class="sc-home">
  ...
  {:else if !soundcloudStore.isLoggedIn}
    <div class="login-card">
      ...
      <h2>{$t("study.music.sc.login_title")}</h2>
      <p>{$t("study.music.sc.login_hint")}</p>
      <div class="login-actions">
        <button type="button" class="cta" disabled={loginBusy} onclick={handleLogin}>
          {loginBusy ? $t("study.music.sc.opening") : $t("study.music.sc.login_btn")}
        </button>
      </div>
      {#if loginError}
        <p class="error">{loginError}</p>
        <details class="why-fail">
          <summary>{$t("study.music.sc.why_fail")}</summary>
          <ul>
            <li>{$t("study.music.sc.fail_new_account")}</li>
            <li>{$t("study.music.sc.fail_2fa")}</li>
            <li>{$t("study.music.sc.fail_captcha")}</li>
          </ul>
        </details>
      {/if}
    </div>
